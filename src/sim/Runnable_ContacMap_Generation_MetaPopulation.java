...
public class Runnable_ContacMap_Generation_MetaPopulation extends Runnable_ClusterModel_ContactMap_Generation_MultiMap {
	...
	public static final String POP_STAT_INTERMITTENT_FORMAT = "POP_STAT_INTERMITTENT_%d.csv"; // Seed
	...
	public void run() {
		if (NUM_GENDER + NUM_LOC + NUM_AGE_GRP > 0) {
			...
			HashMap<Integer, ArrayList<Integer>> schedule_move_grp = new HashMap<>(); // K = time, V = list of pids
				...
				for (int g = 0; g < numInGrp.length; g++) {
					...
					for (int i = 0; i < numInGrp[g]; i++) {
						...
						newPerson[Abstract_Runnable_ClusterModel.POP_INDEX_ENTER_POP_AGE] = (int) ageRange[0]
								+ RNG.nextInt(ageRange[1] - ageRange[2]);

						int exit_grp_time = ageRange[1]
								- (int) newPerson[Abstract_Runnable_ClusterModel.POP_INDEX_ENTER_POP_AGE];
						// Note: Exit at in the case is exit the new group
						newPerson[Abstract_Runnable_ClusterModel.POP_INDEX_EXIT_POP_AT] = exit_grp_time;
						...
						// Active by grp
						ArrayList<Integer> active_by_grp_sorted = active_in_pop.get(g);
						if (active_by_grp_sorted == null) {
							active_by_grp_sorted = new ArrayList<>();
							active_in_pop.put(g, active_by_grp_sorted);
						}
			...
			for (int snapC = 0; snapC < numSnapsSim; snapC++) {
				...
				for (int t = 0; t < snap_dur; t++) {
					...
					ArrayList<Integer> move_grp_pids;

					for (int g = 0; g < numInGrp.length; g++) {
						int[] gla_index = getGrpIndex(g);
						active_by_grp_sorted = active_in_pop.get(g);
						move_grp_pids = schedule_move_grp.remove(popTime);
						for (int move_grp_pid : move_grp_pids) {
							Object[] person_stat = population.get(move_grp_pid);
							int aI = Collections.binarySearch(active_by_grp_sorted, move_grp_pid);
							if (aI >= 0) {
								active_by_grp_sorted.remove(aI);
							}
							if ((gla_index[2] + 1) < NUM_AGE_GRP) {
								// Move to new age grp
								int newGrp = gla_index[2] + 1;
								...
								aI = Collections.binarySearch(target_grp_sorted, move_grp_pid);
								if (aI < 0) {
									target_grp_sorted.add(~aI, move_grp_pid);
								}

							} else {
								// Age out
								pop_stat_final_ids.add(move_grp_pid);
							}
					...
					for (int g = 0; g < numInGrp.length; g++) {
						...
						grpDiff[g] = numInGrp[g] - active_by_grp_sorted.size();
					}

					// TODO: Attempt to get migration for exceed
				...
				try {
					...
					if (!pop_stat_final_ids.isEmpty()) {
						...
						pWri = new PrintWriter(new FileWriter(tarFile, append));
						if (!append) {
							pWri.println("ID,ENTER_POP_AGE,ENTER_POP_AT_TIME,EXIT_POP_AT");
						}
